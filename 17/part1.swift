...
var activePositions = Set<Vec3>() // Stores all positions that are in an 'active' state

// Load the initial state
let input = try! String(contentsOfFile: "input.txt")
input.split(separator: "\n").enumerated().forEach { (y, line) in
	line.enumerated().forEach { (x, ch) in
		if ch == "#" { activePositions.insert(Vec3(x: x, y: y, z: 0)) }
	}
}
...
func adjacentCoords(to pos: Vec3) -> Set<Vec3> {
	var adjacent: Set<Vec3> = []
	for x in -1...1 {
		for y in -1...1 {
			for z in -1...1 {
				guard x != 0 || y != 0 || z != 0 else { continue }
				adjacent.insert(Vec3(x: pos.x + x, y: pos.y + y, z: pos.z + z))
			}
		}
	}
	return adjacent
